...
void main(){
	...
	for(i=0;i<10;i++){
		inputMsg[i]= (char*)malloc(15);
		hash1024SHA1[i]= (char*)malloc(15);
		hash1024SHA256[i]= (char*)malloc(15);
		hash2048SHA1[i]= (char*)malloc(15);
		hash2048SHA256[i]= (char*)malloc(15);

		sprintf(inputMsg[i],"msg/msg%d.msg",i+1);
		sprintf(hash1024SHA1[i],"Output/1024SHA1/cipher.%d",i+1);
		sprintf(hash1024SHA256[i],"Output/1024SHA256/cipher.%d",i+1);
		sprintf(hash2048SHA1[i],"Output/2048SHA1/cipher.%d",i+1);
		sprintf(hash2048SHA256[i],"Output/2048SHA256/cipher.%d",i+1);
	}
